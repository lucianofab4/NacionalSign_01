...
export default function PartiesSignatureMethodSelector({ documentId, party, onUpdated }: Props) {
  ...
  const value = party.requires_certificate ? 'digital' : 'eletronic';

  const handleChange = async (next: 'eletronic' | 'digital') => {
    try {
      setLoading(true);
    ...
    <div className="flex items-center gap-4">
      ...
      <label className="flex items-center gap-1 cursor-pointer">
        <input
          ...
          disabled={loading}
          checked={value === 'eletronic'}
          onChange={() => handleChange('eletronic')}
        />
