...
import { fetchPlans, fetchSubscription, fetchInvoices, createOrUpdateSubscription, seedDefaultPlans, fetchUsage, retryInvoice, type Plan, type Subscription, type Invoice, type Usage } from '../api';
...
export default function BillingPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        const [p, s, i, u] = await Promise.all([
          fetchPlans(),
          fetchSubscription(),
          fetchInvoices(),
          fetchUsage(),
  ...
  return (
    <div className="space-y-6">
      ...
      {invoices.some(inv => (inv.retry_count ?? 0) >= BILLING_MAX_RETRIES && inv.status !== 'paid') && (
      ...
      {invoices.some(inv => (inv.retry_count ?? 0) === BILLING_MAX_RETRIES - 1 && inv.status !== 'paid') && (
            ...
            <div>
              ...
              <div className="w-full bg-slate-100 rounded-md h-3 overflow-hidden">
                <div
                  ...
                  style={{ width: `${usage.documents_percent ?? (usage.documents_quota ? Math.round(usage.documents_used / (usage.documents_quota || 1) * 100) : 0)}%` }}
                />
              </div>
              <div className="text-xs text-slate-600 mt-1">
                {usage.documents_used}/{usage.documents_quota ?? '∞'} neste período
              </div>
            </div>
            <div>
              ...
              <div className="w-full bg-slate-100 rounded-md h-3 overflow-hidden">
                <div
                  className={`h-3 rounded-md ${((usage.users_percent ?? (usage.users_quota ? Math.round(usage.users_used / (usage.users_quota || 1) * 100) : 0)) >= 90) ? 'bg-amber-500' : 'bg-emerald-500'}`}
                  style={{ width: `${usage.users_percent ?? (usage.users_quota ? Math.round(usage.users_used / (usage.users_quota || 1) * 100) : 0)}%` }}
                />
              </div>
              <div className="text-xs text-slate-600 mt-1">
                {usage.users_used}/{usage.users_quota ?? '∞'}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {plans.map(plan => (
            <div key={plan.id} className={`border rounded-lg p-4 ${subscription?.plan_id === plan.id ? 'border-emerald-400' : 'border-slate-200'}`}>
              ...
              <div className="text-slate-600 text-sm">{plan.document_quota} documentos • {plan.user_quota} usuários</div>
